...
int main()
{
    ...
    Student arr_of_students[SIZE_OF_STUDENTS];
    int number_of_students = 0;

    readFileStudents(arr_of_students, number_of_students);

    showMenu(arr_of_students, number_of_students);
...
void readFileStudents(Student* arr_of_students, int& number_of_students)
{
    ...
    if (fin.is_open())
    {
        ...
        while (!fin.eof())
        {
            ...
            {
                cout << "В статическом массиве больше нет места! Зовите программиста!" << endl;
                break;
            }
        }
